	...
	public boolean process(Main main, CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase("auc") || cmd.getName().equalsIgnoreCase("auction")){
			...
			if(args.length == 0) {
				...
				sender.sendMessage("§3Do §b/auc help §3for a list of commands.");
			...
			// If preview
				...
				Player player = (Player) sender;
				main.viewer = Bukkit.createInventory(null, 9, "Auction Preview");
				main.viewer.setItem(4, main.RunningAuction.item.item);
				...
				sender.sendMessage("§3Unknown command: use §c/auc help §3for a list of commands.");
				return true;
			}
